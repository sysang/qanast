...
export class DialogTestClient {
  ...
  private _dialogContext?: DialogContext = undefined;
  ...
  constructor (
    ...
    this.conversationState = conversationState !== undefined ? conversationState : new ConversationState(new MemoryStorage());

    const dialogState = this.conversationState.createProperty('DialogState');
    const options = initialDialogOptions !== undefined ? initialDialogOptions : {};

    if (typeof channelOrAdapter === 'string') {
      const channelIdToUse: string = channelOrAdapter;
      this._callback = this.getDefaultCallback(targetDialog!, options, dialogState);
